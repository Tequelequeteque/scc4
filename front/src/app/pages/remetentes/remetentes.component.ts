...
export class RemetentesComponent implements OnInit {
  private routeName: string = "remetentes";
  ...
  public hiddenFormCadastrar: boolean = false;
  ...
  async handleSubmitCadastrar() {
    ...
    try {
      ...
      this.toast.addToast(
        "success",
        "Sucesso",
        "Remetente cadastrado com sucesso!"
      );
      ...
      if (error.response.status === 400)
        this.toast.addToast(
          "warn",
          "Atenção",
          "Já existe um remetente com esse email."
        );
      else
        this.toast.addToast(
          "error",
